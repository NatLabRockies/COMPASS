...
- Treat `held_out_evals` as a checkpoint you look at occasionally (e.g. before a
  release), not a development loop.

The harness helps enforce this: a `held_out_evals` run writes **only summary
metrics** (`date_extraction_held_out_evals.csv`) — no per-case breakdown, and per-case
